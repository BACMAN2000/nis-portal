/* Comprueba que la traduccion inglesa no deja ninguna linea del castellano
   sin par. Es lo que sostiene la frase "los documentos estan tambien en
   ingles" delante de una head de nivel: si falla, la frase es falsa.

   Recorre TODOS los arcos, no solo los del trimestre en curso, y falla
   tambien si un arco entero se queda sin traducir.
...
Object.keys(A).sort().forEach(k => {
  ...
  if (!b) return mal(k + ': el arco entero esta sin traducir');
  let n = 0, falta = 0;
  a.fases.forEach(f => {
    const o = f.cursos || cu[f.n] || {}, t = (b.semanas || {})[f.n] || {};
    Object.keys(o).forEach(c => {
      n++; tareas++;
      if (!t[c]) { mal(k + ' w' + f.n + ' ' + c + ': sin traducir'); falta++; }
    });
  });
  if (!falta) console.log('   ok ' + k.padEnd(9) + n + ' tareas de curso');
});
...
Object.keys(S).sort().forEach(k => {
  ...
  a.semanas.forEach(w => {
    ...
    Object.keys(w.cursos || {}).forEach(c => {
      n++; tareas++;
      if (!(t.cursos || {})[c]) { mal(k + ' w' + w.n + ' ' + c + ': sin traducir'); falta++; }
    });
    if (w.nota && !t.nota) { mal(k + ' w' + w.n + ': nota sin traducir'); falta++; }
  });
...
console.log('PROPUESTA\n   ok ' + Object.keys(U.grados).length + ' grados, 121 semanas');

console.log(fallos ? '\n' + fallos + ' cosas sin traducir'
  : '\nTraducido y cuadrado: ' + tareas + ' tareas de curso en '
    + (Object.keys(A).length + Object.keys(S).length) + ' arcos, mas la propuesta.');
