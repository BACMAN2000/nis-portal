...
function studentGrade(key){
  ...
  $('#main').innerHTML=`${back}<h1>${emoji} ${label}</h1>
    ...
    <div class="grid cols-2" style="margin-top:12px">
      ...
      ${key==='g9' ? (nodeVisible('english.classes.g9.reader') ? _skillCard('🏝️','And Then There Were None · B1','Agatha Christie at your level: read along with audio, listening, summaries, character files, games and the Detective\'s Notebook.',_withBack('and-then-there-were-none.html?level=b1',route)) : _lockedCard('🏝️','And Then There Were None · B1','Agatha Christie · interactive reader.')) : ''}
      ${key==='g9' ? (nodeVisible('english.classes.g9.reader') ? _skillCard('🐣','And Then There Were None · A2','The same novel in easy English: shorter chapters, slower audio and simpler activities.',_withBack('and-then-there-were-none.html?level=a2',route)) : '') : ''}
      ${key==='g9' ? (nodeVisible('english.classes.g9.reader') ? _skillCard('🎓','And Then There Were None · C1','The advanced retelling: literary prose, inference questions and FCE/CAE-style writing tasks.',_withBack('and-then-there-were-none.html?level=c1',route)) : '') : ''}
    </div>`;
}
