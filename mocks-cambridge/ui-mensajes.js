...
(function () {
  ...
  var CSS = [
    ...
    '.nisui-card{width:min(460px,100%);background:var(--card,#fff);color:var(--text,#0f172a);',
    '  border-radius:16px;padding:26px 26px 22px;box-shadow:0 18px 50px rgba(15,23,42,.28);',
    '  font-family:"Montserrat","Segoe UI",system-ui,sans-serif;line-height:1.55;text-align:center;',
    '  animation:nisui-pop .18s ease-out;border-top:5px solid var(--accent,#4987c6)}',
    ...
    '.nisui-t{font-size:1.15rem;font-weight:700;margin-bottom:8px;color:var(--accent2,#2d5a8d)}',
    '.nisui-card.bien .nisui-t{color:var(--good,#16a34a)}',
    '.nisui-card.mal .nisui-t{color:var(--bad,#ef4444)}',
    '.nisui-m{font-size:.98rem;white-space:pre-line;color:var(--text,#0f172a)}',
    ...
    '  font-family:inherit;background:var(--accent,#4987c6);color:#06283d;transition:transform .08s,opacity .15s}',
    '.nisui-b:hover{transform:translateY(-1px)}',
    '.nisui-b:focus-visible{outline:3px solid var(--soft,#d1d2ea);outline-offset:2px}',
    '.nisui-b.ghost{background:transparent;border:1px solid var(--line,#e2e8f0);color:var(--text,#0f172a)}',
    ...
    '  max-width:min(520px,92vw);padding:13px 20px;border-radius:12px;background:var(--card,#fff);',
    '  color:var(--text,#0f172a);box-shadow:0 12px 34px rgba(15,23,42,.24);border-left:5px solid var(--accent,#4987c6);',
    '  font-family:"Montserrat","Segoe UI",system-ui,sans-serif;font-size:.92rem;white-space:pre-line;',
