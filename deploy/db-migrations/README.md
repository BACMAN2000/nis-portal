...
| 2026-09-21 | `2026-09-21_02_mock_cycles.sql` | 🟢 Funcional | Ciclos de mock: `mock_cycles` (MOCK 1 / MOCK 2, `released_at` = cuándo el alumno puede ver el informe único) y `speaking_results.cycle` (una fila por alumno y ciclo; `upsert_speaking` gana `p_cycle` con default 1). Aplicada por MCP el 21-sep-2026. |
| 2026-09-23 | `2026-09-23_01_speaking_tests.sql` | 🟢 Funcional | Speaking test (Marking → 🗣️ Speaking test): `speaking_tests`, una fila por candidato y sesión (pareja o trío) con las bandas 0-5 de cada criterio de Cambridge, el resultado y la letra AD·A·B·C. RLS: lee el alumno, el admin y el profesor que cubre el grado; escribe admin o profesor con `teacher_covers_student`. Aplicada por MCP el 23-sep-2026. |
| 2026-09-26 | `2026-09-26_01_schools_multi_colegio.sql` | 🟢 Funcional | Multi-colegio, fase 1: `schools`, `apps`, `school_apps`, `profiles.school_id` + `is_superadmin`, trigger `profiles_tenant_guard` (nadie se cambia de colegio ni se asciende a superadmin) y políticas de `profiles` acotadas a «mi colegio». Los 165 perfiles pasan a NIS; nace el colegio `demo` (demo.cohasset.pe). `school_public(host)` es lo único que lee `anon`: marca y apps encendidas. Aplicada por MCP el 26-sep-2026. |
